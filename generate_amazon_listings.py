...
# ---------- CONFIG ----------
BASE_TEMPLATE_PATH = "ELECTRONIC_DEVICE_SKIN.xlsm"
...
def generate_model_workbook(base_bytes, model_row, images_df, chunk_size, template_row_values):
    ...
    if model_images.empty:
        raise ValueError(f"No rows found in images_master.xlsx for model_key='{model_key}'")
    ...
    for idx, design_n in enumerate(design_numbers):
        ...
        image_url = model_images.iloc[idx]["main_image_url"]
        ...
        # Apply every field from MASTER_FIELD_MAP: use model override if present, else base template's value
    ...
    return wb, n_designs, n_parents


def main():
    if not os.path.exists(BASE_TEMPLATE_PATH):
        sys.exit(f"ERROR: base template not found: {BASE_TEMPLATE_PATH}")
    ...
    for _, model_row in models_df.iterrows():
        ...
        try:
            wb, n_designs, n_parents = generate_model_workbook(
